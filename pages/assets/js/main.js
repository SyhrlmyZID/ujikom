...
document.querySelectorAll('.status-dropdown').forEach(select => {
    select.addEventListener('change', function () {
      ...
      xhr.open("POST", "../php/pages/dashboard_buat_tugas/update_status.php", true);
        ...
        if (xhr.status === 200) {
          try {
            ...
            if (response.status === 'success') {
              // Update badge status real-time
              if (statusBadge) {
                statusBadge.innerText = status.charAt(0).toUpperCase() + status.slice(1);
                statusBadge.className = `ml-2 px-2 py-1 text-xs font-semibold capitalize text-white rounded-md 
                              ${status === 'selesai' ? 'bg-green-500' : 'bg-orange-500'}`;
              }
            } else {
...
searchInput.addEventListener('input', function () {
    ...
    typingTimer = setTimeout(() => {
        ...
        // Hapus parameter 'success'
        if (url.searchParams.has('success')) {
            url.searchParams.delete('success');
        }

        url.searchParams.set('search', searchTerm);
        url.searchParams.delete('page'); // Hapus parameter pagination saat searching
        window.location.href = url; // Reload halaman dengan URL baru
    }, typingInterval);
});
...
function openModal(deleteUrl) {
    ...
    confirmDeleteLink.href = deleteUrl;  // Set URL penghapusan
...
function closeModalDelete() {
    ...
    modalDelete.classList.add('hidden');  // Sembunyikan modal
}

// Open the Edit Task Modal and populate data
function openEditTaskModal(element) {
    const modalEdit = document.getElementById('editModal');

    // Ambil nilai dari atribut data-task
    ...
    if (deadline === "Tidak ada tanggal pasti" || deadline === "" || deadline === null) {
        deadlineInput.value = ""; // Kosongkan input date jika tidak ada tanggal
    } else {
